...
import { StyleSheet, Text, View, Button, ScrollView, TextInput, Keyboard, AsyncStorage, Platform, Dimensions, AppState, RefreshControl, Alert, TouchableOpacity } from "react-native";
...
export default class App extends React.Component {
  state = {
    ...
    messages: [
      ...
      {
        id: 3,
        text: "Ai parcat prost si ii incurci pe ceilalti soferi! Ar fi bine sa-ti muti masina!"
      },
      {
        id: 4,
        text: "Ar trebui sa vii la masina! (ai uitat geamul deschis, cheia in contact, bunuri, etc.)"
      },
      {
        id: 5,
        text: "Vin acum!"
  ...
  async componentDidMount() {
    ...
    if (Platform.OS === "android") {
      Expo.Notifications.createChannelAndroidAsync("messages", {
  ...
  sendMessage = async () => {
    ...
    const messagesInLastHourToThisGuy = messagesInLastHour.filter(x => x.targetRegistrationNumber == targetRegistrationNumber);
      ...
      <View style={styles.container} onTouchStart={Keyboard.dismiss}>
        {!registrationNumber ? (
          <View style={{ marginTop: 100, alignItems: "center" }}>
            ...
            <TextInput onChange={event => this.setState({ text: event.nativeEvent.text })} autoCapitalize="characters" style={styles.input} placeholder="e.g. CJ68VGA" />
              ...
              <View style={styles.topWrapper}>
                {!Boolean(targetRegistrationNumber) && (
                  <>
                    ...
                    <TextInput onChange={event => this.setState({ targetText: event.nativeEvent.text })} autoCapitalize="characters" style={styles.input} placeholder="e.g. CJ68VDZ" />
                    <Button color="#00bcd4" disabled={!this.state.targetText} onPress={this.contactDriver} title="Continua" />
                    {Boolean(receivedMessages.length) && <Text style={{ marginTop: 25 }}>Mesaje primite:</Text>}
                    <ScrollView refreshControl={<RefreshControl refreshing={this.state.refreshing} onRefresh={this.loadMessages} />} style={styles.receivedMessagesWrapper}>
                      {receivedMessages
                    ...
                    <ScrollView style={styles.messagesWrapper}>
                      {messages.map(x => (
                        <TouchableOpacity activeOpacity={1} key={x.id} onPress={() => this.selectMessage(x.id)}>
                          <Text style={[styles.message, x.selected ? { backgroundColor: "#00bcd4", fontWeight: "500" } : {}]}>{x.text}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                    <View style={styles.buttonsWrapper}>
                      <Button color="#788190" onPress={this.cancel} title="Inapoi" />
                      <Button disabled={!this.state.messages.some(x => x.selected)} color="#00bcd4" onPress={this.sendMessage} title="Trimite" />
                    </View>
                  </View>
                )}
              </View>
